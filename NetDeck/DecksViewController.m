...
+(void) initialize
{
    sortStr = @{ @(NRDeckListSortByDate): l10n(@"Date"), @(NRDeckListSortByFaction): l10n(@"Faction"), @(NRDeckListSortByName): l10n(@"A-Z") };
...
-(void) changeSideFilter:(id)sender
{
    ...
    self.popup = [UIAlertController actionSheetWithTitle:l10n(@"Show Side") message:nil];
...
-(void) changeStateFilter:(id)sender
{
    ...
    self.popup = [UIAlertController actionSheetWithTitle:l10n(@"Show Status") message:nil];
...
-(void) updateDecks
{
    ...
    if (self.sortType != NRDeckListSortByDate)
    {
    ...
    if (self.filterText.length > 0)
    {
        ...
        NSPredicate* cardPredicate = [NSPredicate predicateWithFormat:@"(ANY cards.card.name CONTAINS[cd] %@) OR (ANY cards.card.englishName CONTAINS[cd] %@)", self.filterText, self.filterText];
