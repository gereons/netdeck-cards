...
- (CGFloat)tableView:(UITableView*)tableView heightForHeaderInSection:(NSInteger)section {
	...
	if ((title = [self tableView:tableView titleForHeaderInSection:section])) {
        CGSize maxSize = CGSizeMake(tableView.frame.size.width - 2*kIASKHorizontalPaddingGroupTitles, INFINITY);
        UIFont* font = [UIFont boldSystemFontOfSize:[UIFont labelFontSize]];
        CGRect rect = [title boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName: font } context:nil];
